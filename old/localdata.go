...
func server(handle func(*Node, *Package), node *Node, conn net.Conn) {
	...
	redirectPackage:
		...
		case role == RelationNode && !setting.HAS_FRIENDS &&
			node.InAccessList(AccessDenied, pack.From.Address):
			return
		case role == RelationNode && setting.HAS_FRIENDS &&
			!node.InAccessList(AccessAllowed, pack.From.Address):
			return
		case node.Setting.ReadOnly == ReadNode && role == RelationHandle:
			return
		case node.Setting.ReadOnly == ReadHandle && role == RelationNode:
			return
		}
		...
		case pack == nil:
			return
		case setting.NETWORK_NAME != pack.Info.NET:
			return
		}
		...
		case setting.TITLE_CONNECT:
			...
			case setting.MODE_REMV:
				node.deleteFromConnection(pack.From.Address)
				if role == RelationNode {
					return
				} else {
					continue
		...
		case setting.TITLE_REDIRECT:
			...
			case setting.MODE_DECENTR:
				...
				if node.inTestConnection(packageID) {
					return
				}
		...
		switch {
		case !node.InConnections(pack.From.Address):
			return
		case role != RelationHidden:
			node.packReceived(pack.From.Address)
		}
...
func (node *Node) onionRouting(pack *Package, role RelationType) *Package {
	if pack == nil || node.IsMyAddress(pack.To.Address) ||
		!node.InConnections(pack.To.Address) {
		return nil
	}
...
func (node *Node) randomRouting(receiver string, role RelationType) []string {
	...
	for addr, conn := range node.Network.Connections {
		...
		if conn.Relation != role {
			continue
		}
		if index == setting.ROUTE_NUM {
			break
		}
