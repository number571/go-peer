package initapp

import (
	"os"
	"testing"

	"github.com/number571/go-peer/pkg/crypto/asymmetric"
)

const (
	tcTestdataPath           = "./testdata/"
	tcTestdataDirPath        = "./testdata/directory"
	tcPrivKey1024Path        = tcTestdataPath + "priv1024.key"
	tcTmpPrivKey512Path      = tcTestdataPath + "tmp_priv512.key"
	tcInvalidPrivKey1024Path = tcTestdataPath + "invalid_priv1024.key"

	tcPasswordPath    = tcTestdataPath + "pasw.txt"
	tcTmpPasswordPath = tcTestdataPath + "tmp_password.txt"

	tcPassword    = "anwdiawdj8u32ndi23hd8uidksdw1ed2" // nolint: gosec
	tcPrivKey1024 = `PrivKey{}`
)

func testDeleteFile(f string) {
	os.RemoveAll(f)
}

func TestError(t *testing.T) {
	t.Parallel()

	str := "value"
	err := &SAppError{str}
	if err.Error() != errPrefix+str {
		t.Error("incorrect err.Error()")
		return
	}
}

func TestGetPassword(t *testing.T) {
	t.Parallel()

	testDeleteFile(tcTmpPasswordPath)
	defer testDeleteFile(tcTmpPasswordPath)

	pasw, err := GetPassword(tcPasswordPath)
	if err != nil {
		t.Error(err)
		return
	}
	if pasw != tcPassword {
		t.Error("diff password")
		return
	}

	if _, err := GetPassword("./random/not_exist/path/57199u140291724y121291d1/pasw.txt"); err == nil {
		t.Error("success get password with not exist directory")
		return
	}
	if _, err := GetPassword(tcTestdataDirPath); err == nil {
		t.Error("success get private key as directory")
		return
	}

	tmpPasw, err := GetPassword(tcTmpPasswordPath)
	if err != nil {
		t.Error(err)
		return
	}
	tmpPasw2, err := GetPassword(tcTmpPasswordPath)
	if err != nil {
		t.Error(err)
		return
	}
	if tmpPasw != tmpPasw2 {
		t.Error("diff temp passwords")
		return
	}
}

func TestGetPrivKey(t *testing.T) {
	t.Parallel()

	testDeleteFile(tcTmpPrivKey512Path)
	defer testDeleteFile(tcTmpPrivKey512Path)

	privKey, err := GetPrivKey(1024, tcPrivKey1024Path)
	if err != nil {
		t.Error(err)
		return
	}
	if privKey.ToString() != asymmetric.LoadRSAPrivKey(tcPrivKey1024).ToString() {
		t.Error("diff private keys")
		return
	}

	if _, err := GetPrivKey(1024, tcInvalidPrivKey1024Path); err == nil {
		t.Error("success get invalid private key")
		return
	}
	if _, err := GetPrivKey(2048, tcPrivKey1024Path); err == nil {
		t.Error("success get private key with diff size")
		return
	}
	if _, err := GetPrivKey(512, "./random/not_exist/path/57199u140291724y121291d1/priv.key"); err == nil {
		t.Error("success get private key with not exist directory")
		return
	}
	if _, err := GetPrivKey(512, tcTestdataDirPath); err == nil {
		t.Error("success get private key as directory")
		return
	}

	tmpPrivKey, err := GetPrivKey(512, tcTmpPrivKey512Path)
	if err != nil {
		t.Error(err)
		return
	}
	tmpPrivKeyX, err := GetPrivKey(512, tcTmpPrivKey512Path)
	if err != nil {
		t.Error(err)
		return
	}
	if tmpPrivKey.ToString() != tmpPrivKeyX.ToString() {
		t.Error("diff tmp private keys")
		return
	}
}
