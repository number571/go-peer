...
func (p *sClient) DistributeRequest(pCtx context.Context, pRequest request.IRequest) (response.IResponse, error) {
	res, err := p.fRequester.DistributeRequest(pCtx, pRequest)
	if err != nil {
		return nil, fmt.Errorf("get settings (client): %w", err)
	}
	return res, nil
