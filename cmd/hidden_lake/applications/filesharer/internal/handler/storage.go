...
func StoragePage(pLogger logger.ILogger, pCfg config.IConfig, pPathTo string) http.HandlerFunc {
	return func(pW http.ResponseWriter, pR *http.Request) {
		...
		if pR.FormValue("method") == http.MethodPost {
			fileName = downloadFile(pLogger, pCfg, pPathTo, pW, pR, aliasName, hlsClient, hlfClient)
			if fileName == "" {
				return
			}
		}
...
func downloadFile(
	...
	chunkSize, err := getMessageLimit(pHlsClient)
	if err != nil {
	...
	for i := uint64(startChunk); i < chunksCount; i++ {
		for j := uint64(0); j <= retryNum; j++ {
			chunk, err := pHlfClient.LoadFileChunk(pAliasName, fileName, i)
			if err != nil {
				if j == retryNum {
