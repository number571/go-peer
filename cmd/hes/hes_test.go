...
const (
	...
	tcPatternTitleHES = "message-is-email"
...
func testClientDoPush() error {
	...
	for i := 0; i < tcN; i++ {
		encEmail, _ := client.Encrypt(
			local.NewRoute(tgPubKeyReceiver, nil, nil),
			local.NewMessage(
				[]byte(tcPatternTitleHES),
		...
		request := struct {
			...
			Receiver: hashRecv,
			Package:  encEmail.ToPackage().Bytes(),
		}
...
func testClientDoLoad() error {
	...
	for i := 0; i < tcN; i++ {
		...
		if string(title) != tcPatternTitleHES {
