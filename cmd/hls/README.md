# HLS

> Hidden Lake Service.

## Description

HLS is the core of the anonymous network, representing API functions for interacting with the network.

## Config structure

```json
{
	"address": {
        // Connection address for anonymity network, may be void
		"tcp": "localhost:9571",
        // Connection address for API functions
		"http": "localhost:9572"
	},
	"services": {
        // Redirect request from network to service
		"hidden-default-service": "localhost:8080"
	},
	"connections": [
        // Connection addresses of the another nodes in network
		"localhost:9571"
	],
	"friends": [
        // Friend addresses for send or receive messages over network
		"Pub(go-peer/rsa){}"
	]
}
```

## Request structure in HLS

```json
// need encode this json to hex format
// and put this to "hex_data" HLS API
{
	"method":"GET",
	"host":"hidden-default-service",
	"path":"/",
	"head":{
		"Accept": "application/json"
	},
	"body":"aGVsbG8sIHdvcmxkIQ==" // base64
}
```

## Response structure in HLS

```json
{
	"result":"", // string;
	"return":1 	 // int; 1 = success
}
```

## API

1. Get current connections
2. Get friends from config
3. Get public key of HLS node
4. Put message to network without response (broadcast)
5. Put message to network with response (request)

### 1. Get current connections

#### Request

```bash
curl -i -H 'Accept: application/json' http://localhost:9572/network/online
```

#### Response
```json
HTTP/1.1 200 OK
Content-Type: application/json
Date: Fri, 07 Oct 2022 12:09:53 GMT
Content-Length: 55

{"result":"127.0.0.1:35370,127.0.0.1:9571","return":1}
```

### 2. Get friends from config

#### Request

```bash
curl -i -H 'Accept: application/json' http://localhost:9572/node/friends
```

#### Response
```json
HTTP/1.1 200 OK
Content-Type: application/json
Date: Fri, 07 Oct 2022 12:12:03 GMT
Content-Length: 323

{"result":"Pub(go-peer/rsa){}","return":1}
```

### 3. Get public key of HLS node

#### Request

```bash
curl -i -H 'Accept: application/json' http://localhost:9572/node/pubkey
```

#### Response
```json
HTTP/1.1 200 OK
Content-Type: application/json
Date: Fri, 07 Oct 2022 12:13:21 GMT
Content-Length: 1095

{"result":"Pub(go-peer/rsa){}","return":1}
```

### 4. Put message to network without response (broadcast)

#### Request

```bash
curl -i -X PUT -H 'Accept: application/json' http://localhost:9572/do/broadcast --data '{"receiver":"Pub(go-peer/rsa){}","hex_data":"68656C6C6F2C20776F726C6421"}'
```

#### Response
```json
HTTP/1.1 200 OK
Content-Type: application/json
Date: Fri, 07 Oct 2022 12:16:46 GMT
Content-Length: 43

{"result":"success: broadcast","return":1}
```

### 5. Put message to network with response (request)

#### Request

```bash
#!/bin/bash

str2hex() {
    local str=${1:-""}
    local fmt="%02X"
    local chr
    local -i i
    for i in `seq 0 $((${#str}-1))`; do
        chr=${str:i:1}
        printf "${fmt}" "'${chr}"
    done
}

JSON_DATA='{
        "method":"POST",
        "host":"hidden-echo-service",
        "path":"/echo",
        "head":{
            "Accept": "application/json"
        },
        "body":"aGVsbG8sIHdvcmxkIQ=="
}';

REQUEST_FORMAT="{
        \"receiver\":\"Pub(go-peer/rsa){}\",
        \"hex_data\":\"$(str2hex "$JSON_DATA")\"
}";

curl localhost:9572/do/request -X PUT --data "${REQUEST_FORMAT}"
```

#### Response
```json
HTTP/1.1 200 OK
Content-Type: application/json
Date: Fri, 07 Oct 2022 12:18:34 GMT
Content-Length: 49

{"result":"7b226563686f223a2268656c6c6f2c20776f726c6421222c2272657475726e223a317d0a","return":1}
```
