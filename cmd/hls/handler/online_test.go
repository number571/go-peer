...
func TestHandleOnlineAPI(t *testing.T) {
	...
	client := hlc.NewClient(
		hlc.NewRequester(fmt.Sprintf("http://%s", testutils.TgAddrs[12])),
	)
...
func testGetOnlines(t *testing.T, client hlc.IClient, node anonymity.INode) {
...
func testDelOnline(t *testing.T, client hlc.IClient, addr string) {
